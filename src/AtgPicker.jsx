...
import{searchAtgClassifier}from'./services/atgClassifier';
...
export default function AtgPicker({value='',onSelect,onClose}){
 ...
 const runSearch=async q=>{setLoading(true);setError('');try{const rows=await searchAtgClassifier(q,30);setItems(rows)}catch(e){setError(e.message||'Не удалось загрузить классификатор ԱՏԳԱԱ')}finally{setLoading(false)}};
 useEffect(()=>{runSearch(value||'')},[]);
 const change=e=>{const q=e.target.value;setQuery(q);runSearch(q)};
