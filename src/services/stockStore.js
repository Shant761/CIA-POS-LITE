...
let toastTimer=null;
const showCartToast=()=>{if(typeof document==='undefined')return;let el=document.getElementById('cia-cart-toast');if(!el){el=document.createElement('div');el.id='cia-cart-toast';el.className='cia-cart-toast';document.body.appendChild(el)}el.innerHTML='<span>✓</span><b>Товар добавлен</b>';el.classList.add('show');clearTimeout(toastTimer);toastTimer=setTimeout(()=>el.classList.remove('show'),900)};
...
export function canTakeStock(productId,qty=1,alreadyInCart=0){const settings=getStockSettings(),available=getStock(productId),requested=Number(alreadyInCart||0)+Number(qty||0);const ok=settings.allowNegative||requested<=available;if(ok&&Number(qty)>0)showCartToast();return{ok,available,requested}}
